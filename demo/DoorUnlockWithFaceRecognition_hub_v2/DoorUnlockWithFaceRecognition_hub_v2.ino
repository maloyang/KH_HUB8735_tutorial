...
// Pin Definition
#define RED_LED    2 //p2(A1)
#define GREEN_LED  3 //p3(A0)
#define BUTTON_PIN 5 //p5(A2)
#define SERVO_PIN  8 //F7
#define MY_HIGH 0
#define MY_LOW 1
#define ANG_LOCK 90
#define ANG_OPEN 0
...
void setup() {
    ...
    myservo.write(ANG_LOCK); //改成90度LOCK //myservo.write(180);
}
...
void loop() {
    ...
    if ((buttonState == HIGH) && (systemOn == false)) {  // When button is being pressed, systemOn becomes true
        for (int count = 0; count < 3; count++) {
            digitalWrite(RED_LED, MY_HIGH);
            digitalWrite(GREEN_LED, MY_HIGH);
            delay(500);
            digitalWrite(RED_LED, MY_LOW);
            digitalWrite(GREEN_LED, MY_LOW);
            delay(500);
        }
        systemOn = true;
    ...
    if ((doorOpen == true) && (systemOn == true)) {
        ...
        file.close();
        fs.end();
        myservo.write(ANG_OPEN); //myservo.write(0);
        Serial.println("Opening Door!");
        
        delay(10000);
        myservo.write(ANG_LOCK); //myservo.write(180);
        digitalWrite(RED_LED, MY_HIGH);
        digitalWrite(GREEN_LED, MY_LOW);
        doorOpen = false;
    }
